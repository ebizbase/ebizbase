...
describe('LocalizeController', () => {
  ...
  describe('listLanguages', () => {
    it('should return an observable of languages', (done) => {
      const mockLanguages = [
        { id: '1', name: 'English', code: 'en', isDefault: true, createdAt: new Date(), updatedAt: new Date() },
        { id: '2', name: 'Spanish', code: 'es', isDefault: false, createdAt: new Date(), updatedAt: new Date() }
      ];
  ...
  describe('changeDefaultLanguage', () => {
    it('should return an observable of google.protobuf.Empty', (done) => {
      jest.spyOn(languageService, 'changeDefaultLanguage').mockImplementation(() => Promise.resolve());
      localizeController.changeDefaultLanguage({ id: '1' } as localize.ChangeDefaultLanguageRequest).subscribe((result) => {
  ...
  describe('listTranslations', () => {
    it('should return an observable of translations', async () => {
      const mockTranslations: Array<Translation> = [
        { service: 'test', languageCode: '1', key: 'key1', value: 'value1', createdAt: new Date(), updatedAt: new Date() },
        { service: 'test', languageCode: '1', key: 'key2', value: 'value2', createdAt: new Date(), updatedAt: new Date() }
      ];
      jest.spyOn(languageService, 'listTranslations').mockResolvedValue(mockTranslations);

      const result = await firstValueFrom(localizeController.listTranslations({} as common.Listing));
  ...
  describe('updateTranslation', () => {
    it('should return an observable of translations', (done) => {
      ...
      localizeController.updateTranslation({} as localize.UpdateTranslactionRequest).subscribe((result) => {
        expect(result).toEqual({});
        done();
      });
    });
