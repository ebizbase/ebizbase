...
describe('Main E2e Spec', () => {
  ...
  beforeAll(async () => {
    ...
    mongoContainer = await new MongoDBContainer('docker.io/mongo:4.4')
      .start();
  ...
  describe('Change default language', () => {
    it('should return all language', async () => {
      ...
      expect(newResults.languages.find((language) => language.isDefault).id).toBe(newDefaultLanguage.id);
    });
  });
});
