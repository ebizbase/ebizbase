...
export class EbbCookie {
  ...
  check(name: string): boolean {
    ...
    const regExp: RegExp = EbbCookie.getCookieRegExp(name);
  ...
  get(name: string): string {
    ...
    if (!cookie) {
      ...
      return result && result[1] ? EbbCookie.safeDecodeURIComponent(result[1]) : '';
    }
  }
  ...
  getAll(): Dict<string> {
    ...
    if (cookieString && cookieString !== '') {
      cookieString.split(';').forEach((currentCookie) => {
        ...
        cookies[EbbCookie.safeDecodeURIComponent(cookieName.replace(/^ /, ''))] =
          EbbCookie.safeDecodeURIComponent(cookieValue);
      });
    }
