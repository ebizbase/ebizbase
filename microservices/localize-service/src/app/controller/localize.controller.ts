...
export class LocalizeController implements localize.LocalizeService {
  ...
  changeDefaultLanguage(
    { id }: localize.ChangeDefaultLanguageRequest,
    metadata?: Metadata
  ): Observable<google.protobuf.Empty> {
    return from(this.languageService.changeDefaultLanguage(id, metadata)).pipe(map(() => ({})));
  }

  listTranslations(data: common.Listing, metadata?: Metadata): Observable<localize.Translations> {
    return from(this.languageService.listTranslations(data, metadata)).pipe(
      map((translations) => ({ translations }))
    );
  }

  updateTranslation(
    data: localize.UpdateTranslactionRequest,
    metadata?: Metadata
  ): Observable<localize.Translations> {
    return from(this.languageService.updateTranslation(data, metadata)).pipe(map(() => ({})));
  }
}
